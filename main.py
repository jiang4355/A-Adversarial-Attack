...
def attack(args):
    ...
    for i, sample in enumerate(x_test[:args.num_samples]):
        ...
        perturbed,dist = myAttack(model, 
                            ...
                            max_num_evals = 1e4,
                            init_num_evals = 100,
                            select_orthogonal_type=args.select_orthogonal_type,
                            spherical_stepsize=args.spherical_stepsize)
        image = np.concatenate([sample, np.zeros((32,8,3)), perturbed], axis = 1)
        scipy.misc.imsave('{}/figs-iter{}-{}-{}/my-{}-i{}-{}-{}-dist{}-step{}.jpg'.format(data_model, args.num_iterations,args.select_orthogonal_type,args.attack_type,
            args.attack_type, i,label,target_label,str(dist)[0:5].replace('.','e'),str(args.spherical_stepsize).replace('.','e')), image)
        print('hsja:')
        perturbed,dist = hsja(model, 
                            sample, 
                            clip_max = 1, 
                            clip_min = 0, 
                            constraint = args.constraint, 
        ...
        image = np.concatenate([sample, np.zeros((32,8,3)), perturbed], axis = 1)
        scipy.misc.imsave('{}/figs-iter{}-hsja/hsja-{}-i{}-{}-{}-dist{}.jpg'.format(data_model, args.num_iterations,
            args.attack_type, i,label,target_label,str(dist)[0:5].replace('.','e')), image)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--select_orthogonal_type',type=str,
                       choices = ['randomly','minimum','minDy','spsa'],
                       default='randomly')
    
    parser.add_argument('--spherical_stepsize',type=float,
                       default=0.01)
    ...
    if not os.path.exists(data_model):
        os.mkdir(data_model)
    if not os.path.exists('{}/figs-iter{}-{}-{}'.format(data_model,args.num_iterations,args.select_orthogonal_type,args.attack_type)):
        os.mkdir('{}/figs-iter{}-{}-{}'.format(data_model,args.num_iterations,args.select_orthogonal_type,args.attack_type))
        
    if not os.path.exists('{}/figs-iter{}-hsja'.format(data_model,args.num_iterations)):
        os.mkdir('{}/figs-iter{}-hsja'.format(data_model,args.num_iterations))
